...
<?php if ($model->getRole() == GroupUser::ROLE_INVITING): ?>
    <h3>Mời tham gia nhóm: </h3>
    <h4><?= Yii::$app->formatter->asHtml($model->description) ?></h4>
    <?= Html::a('Apply to join', ['/group/accept', 'id' => $model->id, 'accept' => 1], ['class' => 'btn btn-success']) ?>
...
<?php elseif ($model->join_policy == Group::JOIN_POLICY_APPLICATION): ?>
    <?= Html::a('Register member', ['/group/accept', 'id' => $model->id, 'accept' => 3], ['class' => 'btn btn-success']) ?>
<?php elseif ($model->join_policy == Group::JOIN_POLICY_FREE): ?>
    <?= Html::a('Join group', ['/group/accept', 'id' => $model->id, 'accept' => 2], ['class' => 'btn btn-success']) ?>
<?php endif; ?>
