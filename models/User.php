...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
            ...
            ['username', 'match', 'pattern' => '/^(?!c[\d]+user[\d])/', 'message' => 'Use c+number+user+number as the account name system reserved', 'when' => function($model) {
                return $model->role != User::ROLE_PLAYER;
            }],
            ['username', 'unique', 'targetClass' => '\app\models\User', 'message' => 'Tên người dùng đã được sử dụng.'],
    ...
    public function validateOldPassword()
    {
        ...
        if (!$user || !$user->validatePassword($this->oldPassword)) {
            Yii::$app->getSession()->setFlash('error', 'Mật khẩu cũ không chính xác.');
            $this->addError('password', 'Mật khẩu cũ không chính xác.');
        }
    }
