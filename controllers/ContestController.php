...
class ContestController extends BaseController
{
    ...
    public function actionPrint($id)
    {
        ...
        if ($model->scenario != Contest::SCENARIO_OFFLINE || $model->getRunStatus() == Contest::STATUS_ENDED) {
            throw new ForbiddenHttpException('Tuỳ chọn không có sẵn trong cuộc thi này.');
        }
        ...
        if (!Yii::$app->user->isGuest && $newClarify->load(Yii::$app->request->post())) {
            ...
            if ($discuss !== null) {
                if (empty($newClarify->content)) {
                    Yii::$app->session->setFlash('error', 'Nội dung không được để trống');
                    return $this->refresh();
                }
                ...
                Yii::$app->session->setFlash('error', 'Tiêu đề không được để trống');
                return $this->refresh();
            }
            ...
            try {
                ...
                $content = 'Cuộc thi: ' . $model->title .  ' - vui lòng truy cập wiki để biết thêm thông tin chi tiết. ';
                foreach ($uids as $uid) {
    ...
    public function actionProblem($id, $pid = 0)
    {
        ...
        if (!Yii::$app->user->isGuest && $solution->load(Yii::$app->request->post())) {
            ...
            if ($model->getRunStatus() == Contest::STATUS_NOT_START) {
                Yii::$app->session->setFlash('error', 'Cuộc thi chưa bắt đầu.');
                return $this->refresh();
            }
            if ($model->isContestEnd() && time() < strtotime($model->end_time) + 5 * 60) {
                Yii::$app->session->setFlash('error', 'Cuộc thi đã kết thúc. Các bài nộp trong cuộc thi sẽ được mở sau 05 phút. ');
                return $this->refresh();
            }
