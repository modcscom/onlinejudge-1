...
class GroupController extends BaseController
{
    ...
    public function actionAccept($id, $accept = -1)
    {
        ...
        if ($model->join_policy == Group::JOIN_POLICY_INVITE && $model->getRole() != GroupUser::ROLE_INVITING) {
            throw new ForbiddenHttpException('Không được thực hiện thao tác này.');
        }
        ...
        if (!Yii::$app->user->isGuest) {
            ...
            if ($accept == 0 && $role == GroupUser::ROLE_INVITING) { 
                ...
                Yii::$app->session->setFlash('info', 'Tham gia');
                ...
                Yii::$app->session->setFlash('info', 'Đã áp dụng');
            }
        }
    ...
    public function actionView($id)
    {
        ...
        if ($newGroupUser->load(Yii::$app->request->post())) {
            ...
            if (!isset($query['user_id'])) {
                ...
                $newGroupUser->save();
                Yii::$app->session->setFlash('success', 'Đã mời');
            } else {
                Yii::$app->db->createCommand()->update('{{%group_user}}', [
                    'role' => GroupUser::ROLE_INVITING
                ], ['user_id' => $query['user_id'], 'group_id' => $model->id])->execute();
                Yii::$app->session->setFlash('error', 'Đã mời');
            }
            return $this->refresh();
        }
    ...
    protected function findModel($id)
    {
        ...
        throw new NotFoundHttpException(Yii::t('app', 'Trang được yêu cầu không tồn tại.'));
    }
}
